...
void sm_crypt_ecb_16( sm4_context *ctx, int mode,  int length, unsigned char *input,unsigned char *output)
{
	int ilen = (length >> 4)<<4;
	//char* cleft
	sm4_crypt_ecb(ctx,mode,ilen,input,output);
	memcpy(&output[ilen],&input[ilen],length-ilen);
	return ;
}
