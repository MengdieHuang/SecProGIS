...
struct ListCtrlSortHelper
{
	...
	{
		static CRegexp reg((unsigned short *)MATCH_NUMBERS);
		int nEnd1 = 0, nEnd2 = 0;
		
		while(1)
		{
			char chText[MAX_PATH];
			char chText2[MAX_PATH];
			//char *pubKeyFilePath;
			memset(chText,0,MAX_PATH);
			memset(chText2,0,MAX_PATH);

			int nLength = csText1.GetLength();
			int nBytes = WideCharToMultiByte(CP_ACP,0,csText1,nLength,NULL,0,NULL,NULL);
			WideCharToMultiByte(CP_OEMCP, 0, csText1, nLength, chText, nBytes, NULL, NULL); 
			chText[nBytes] = 0;

			nLength = csText2.GetLength();
			nBytes = WideCharToMultiByte(CP_ACP,0,csText2,nLength,NULL,0,NULL,NULL);
			WideCharToMultiByte(CP_OEMCP, 0, csText2, nLength, chText2, nBytes, NULL, NULL); 
			chText2[nBytes] = 0;

			MatchResult ret1 = reg.Match((unsigned short *)chText, nEnd1);
			MatchResult ret2 = reg.Match((unsigned short *)chText2, nEnd2);
...
class ListViewSort : public CListCtrl, public ListCtrlSortHelper<ListViewSort>
{
	void OnColumnClick(NMHDR* pNMHDR, LRESULT* pResult ) 
